...
function buildReleaseTag(version: string | null): string | null {
  return version ? `v${version}` : null;
}

async function ensureLocalTag(options: {
  cwd: string;
  tagName: string;
  message: string;
}): Promise<boolean> {
  try {
    await runGit(options.cwd, ["rev-parse", `refs/tags/${options.tagName}`]);
    return false;
  } catch {
    await runGit(options.cwd, ["tag", "-a", options.tagName, "-m", options.message]);
    return true;
  }
}
...
export async function performGitHubDeliverMutations(options: PerformGitHubMutationOptions): Promise<PerformGitHubMutationResult> {
  ...
  const releaseRequested = Boolean(policy.createRelease || policy.releasePushTag || policy.mode === "release");
  ...
  const releaseTag = buildReleaseTag(packageVersion);
  let releaseCreated = false;
  let releasePushed = false;
  let releaseUrl: string | undefined;
  let releaseName: string | null | undefined;
  const uploadedReleaseFiles: string[] = [];
  const blockers: string[] = [];
    ...
    return {
      ...
      record: {
        ...
        release: {
          ...
          pushed: false,
          uploadedFiles: [],
          summary: "Release mutation is disabled by policy."
        },
        blockers: [],
        summary: "GitHub mutations are disabled by policy."
      }
  ...
  if (releaseRequested && blockers.length === 0) {
    if (!releaseTag) {
      blockers.push("package.json version could not be read, so cstack could not derive a release tag.");
    } else {
      try {
        releaseCreated = await ensureLocalTag({
          cwd: options.cwd,
          tagName: releaseTag,
          message: policy.releaseMessage || `Release ${releaseTag}`
        });
      } catch (error) {
        const failure = classifyGitFailure(`creating local tag ${releaseTag}`, error);
        blockers.push(failure.summary);
        blockers.push(failure.detail);
      }
    }
  ...
  if (policy.releasePushTag && blockers.length === 0 && releaseTag) {
    if (!remote) {
      blockers.push("No git remote is configured for pushing release tags.");
    } else {
      try {
        await runGit(options.cwd, ["push", remote, releaseTag]);
        releasePushed = true;
      } catch (error) {
        const failure = classifyGitFailure(`pushing release tag ${releaseTag}`, error);
        blockers.push(failure.summary);
        blockers.push(failure.detail);
      }
    }
  ...
  if (policy.createRelease && blockers.length === 0) {
    if (!repository) {
      blockers.push("GitHub repository could not be resolved for release mutation.");
    } else if (!releaseTag) {
      blockers.push("No release tag could be derived for GitHub release creation.");
    } else {
      const releaseNotesPath = path.join(path.dirname(options.pullRequestBodyPath), "release-notes.md");
      const releaseNotesBody = policy.releaseMessage?.trim()
        ? policy.releaseMessage
        : [
            `Release generated by cstack for ${releaseTag}.`,
            "",
            options.buildSummary,
            "",
            `Review: ${options.reviewVerdict.summary}`
          ].join("\n");
      await fs.writeFile(releaseNotesPath, `${releaseNotesBody}\n`, "utf8");

      try {
        const existingRelease = await runGh(ghCommand, options.cwd, [
          "release",
          "view",
          releaseTag,
          "--repo",
        ...
        if (!existingRelease) {
          const createArgs = [
            ...
            repository,
            "--title",
            policy.releaseName || releaseTag,
            ...(policy.releaseDraft ? ["--draft"] : []),
            ...(policy.releasePrerelease ? ["--prerelease"] : []),
            ...(policy.releaseGenerateNotes === false ? ["--notes-file", releaseNotesPath] : ["--generate-notes"])
          ];
          const releaseFiles = (policy.releaseFiles ?? [])
            .map((file) => path.resolve(options.cwd, file))
            .filter((filePath) => filePath && filePath !== options.cwd);
          for (const filePath of releaseFiles) {
            if (await pathExists(filePath)) {
              createArgs.push(filePath);
              uploadedReleaseFiles.push(path.relative(options.cwd, filePath));
            }
          }
          await runGh(ghCommand, options.cwd, createArgs);
          releaseCreated = true;
        } else {
          ...
          for (const relativeFile of policy.releaseFiles ?? []) {
            const filePath = path.resolve(options.cwd, relativeFile);
            if (!(await pathExists(filePath))) {
              continue;
            }
            await runGh(ghCommand, options.cwd, ["release", "upload", releaseTag, filePath, "--repo", repository, "--clobber"]);
            uploadedReleaseFiles.push(path.relative(options.cwd, filePath));
          }
        }

        const { stdout } = await runGh(ghCommand, options.cwd, [
          "release",
          "view",
          releaseTag,
          "--repo",
        ...
        const releasePayload = JSON.parse(stdout) as { url?: string; name?: string | null };
        releaseUrl = releasePayload.url;
        releaseName = releasePayload.name ?? policy.releaseName ?? releaseTag;
      } catch (error) {
        const failure = classifyGitHubFailure(`creating or updating release ${releaseTag}`, error);
        blockers.push(failure.summary);
        blockers.push(failure.detail);
      }
    }
  ...
  return {
    ...
    record: {
      ...
      release: {
        ...
        ...(releaseTag ? { tagName: releaseTag } : {}),
        ...(packageVersion ? { version: packageVersion } : {}),
        created: releaseCreated,
        pushed: releasePushed,
        uploadedFiles: uploadedReleaseFiles,
        ...(releaseUrl ? { url: releaseUrl } : {}),
        ...(releaseName !== undefined ? { name: releaseName } : {}),
        summary:
          !releaseRequested
            ? "Release mutation not requested."
            : blockers.length === 0
              ? `Release ${releaseTag ?? "(unknown tag)"} is prepared.`
              : `Release mutation encountered blockers for ${releaseTag ?? "(unknown tag)"}.`
      },
      blockers,
      summary:
        blockers.length === 0
          ? releaseUrl
            ? `Branch ${branch} is prepared and release ${releaseUrl} is available.`
