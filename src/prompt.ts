...
export async function buildDeliverValidationLeadPrompt(options: {
  ...
  return {
    prompt: [
      ...
      "",
      "Requirements:",
      "- validation is read-only: do not edit repository files, apply patches, run git writes, or create repo files outside stage artifacts",
      "- do not install, download, or upgrade tools during this validation run; use only tools already present on the host",
      "- refine the validation plan only; do not turn validation into a second broad implementation pass",
      ...
      "- keep the validation scope bounded to a first slice; if more commands or CI jobs would help, defer them explicitly instead of expanding scope",
      ...
      '  "deferredScope": ["scope item"],',
      '  "classificationReason": "why this validation result was classified this way",',
